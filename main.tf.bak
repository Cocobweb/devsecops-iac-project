...
# FIX CKV2_AWS_11 : VPC Flow Logs vers CloudWatch
resource "aws_cloudwatch_log_group" "vpc_flow_logs" {
  #checkov:skip=CKV_AWS_158: Chiffrement KMS hors scope sandbox — chiffré par clés AWS managées. CMK requise en production.
...
# 3️ Subnet public (pour Internet Gateway uniquement — pas d'EC2 ici)
...
# 6️ Route table privée pour le subnet EC2
...
# 7️ VPC Endpoint S3 Gateway — yum + layers Docker sans internet

# S3 Interface endpoint — résolution DNS privée pour ECR layers + yum
...
# 8️ VPC Endpoints SSM — accès SSH-less à l'EC2
...
# 9️ VPC Endpoints ECR — docker pull depuis l'EC2 sans internet
...
resource "aws_security_group" "sg" {
  #checkov:skip=CKV_AWS_260: Port 80 ouvert publiquement — serveur web public.
  name_prefix = "web-http-only-"
  description = "Allow HTTP only, SSM access via VPC endpoints"
  vpc_id      = aws_vpc.main.id
  ...
  egress {
    description = "Allow HTTPS outbound to VPC for SSM and ECR endpoints"
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
...
# 12️ EC2 Instance dans subnet privé
...
# 16️ IAM Role CloudTrail -> CloudWatch
